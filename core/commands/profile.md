...
> **Forward the `<<<FORGE_HANDOFF>>>` block verbatim into each spawned `performance-profiler`'s prompt** so they use the already-loaded profile context instead of re-reading `.forge.yaml`.
